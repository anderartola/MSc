...
class NeuralNetworkTrainer:
    ...
    def train(self, save_weights, epochs, patience_epochs, 
        ...
        with self.strategy.scope():
            self.optimizer = tf.keras.optimizers.legacy.Adam(self.lr)
        ...
        #Changed by me
        np.save(self.output_dir+"loss_train_nll.npy",self.loss_nll_list)
        np.save(self.output_dir+"loss_test_nll.npy",self.test_loss_nll_list)
        
        np.save(self.output_dir+"loss_train_mae.npy",self.mae_list)
        np.save(self.output_dir+"loss_test_mae.npy",self.test_mae_list)
        
        # plt.plot(self.loss_nll_list,label="Train")
        # plt.plot(self.test_loss_nll_list,label="Test")
        # plt.legend()
        # plt.show()
    ...
    def predict(self, dpp):
        ...
        for mi, file in enumerate(self.files_list):
            ...
            # print(list(map(lambda x: x.name, self.ml_model.layers)))
            # intermediate_output = tf.keras.Model(self.ml_model.inputs, 
            #                         self.ml_model.get_layer('resnet_block_6').outputs)

            # extracted = intermediate_output(tf.convert_to_tensor(
            #     self.normalize(x, self.xscalar_mean, self.xscalar_var)), 
            #     training=False)
            # print("Intermediate output",extracted)
