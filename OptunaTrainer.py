...
class OptunaTrainer:
    ...
    def suggest_hyperparams(self):
        ...
        self.lr = self.trial.suggest_float('lr', 1e-4, 0.1, log=True)
        # batch size and features per block in power od two
        self.batch_size = 2**self.trial.suggest_int("batch_size", 4, 8)
        self.num_blocks = self.trial.suggest_int("num_blocks", 1, 4)
