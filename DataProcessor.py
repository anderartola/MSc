...
class DataProcessor:
    ...
    def stack_dataset(self) -> NoReturn:
        ...
        for mi, filename in enumerate(self.files_list):
            ...
            #Shuffle before stacking inside of each model's dataset!
            np.random.seed(12345)#SET THE SEED 
            index_shuffle=np.random.choice(range(self.x.shape[0]),size=self.x.shape[0],replace=False)
            self.x=self.x[index_shuffle]
            self.y=self.y[index_shuffle]
            self.w=self.w[index_shuffle]
            #------------------------------------
